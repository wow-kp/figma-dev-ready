...
export async function runAudit(){
  ...
  async function walk(node, depth, insideInst) {
    ...
    if(!insideInst && (node.type==="FRAME"||node.type==="COMPONENT")&&node.layoutMode!=="NONE"){var unboundProps=[];var firstSpVal=0;var _spProps=["paddingLeft","paddingRight","paddingTop","paddingBottom","itemSpacing"];for(var _spi2=0;_spi2<_spProps.length;_spi2++){var prop=_spProps[_spi2];if(!(prop in node))continue;var val=node[prop];if(val===figma.mixed||!val||val<=0)continue;var b=bv[prop];if(!b){unboundProps.push(prop.replace(/([A-Z])/g," $1").toLowerCase());if(!firstSpVal)firstSpVal=val;}}if(unboundProps.length){var nv=await findNearestFloatVar(firstSpVal,floatVars);checks.spacingVars.issues.push({id:node.id,label:'"'+trunc(node.name)+'" — '+unboundProps.join(", "),path:path,suggestedVar:await varInfo(nv),rawValue:firstSpVal,bindType:"spacing"});}}
    if("cornerRadius"in node&&node.cornerRadius!==figma.mixed&&node.cornerRadius>0){var b=bv.cornerRadius||bv.topLeftRadius||bv.topRightRadius||bv.bottomLeftRadius||bv.bottomRightRadius;if(!b){var nv=await findNearestFloatVar(node.cornerRadius,floatVars);checks.radiusVars.issues.push({id:node.id,label:'"'+trunc(node.name)+'" — '+node.cornerRadius+'px',path:path,suggestedVar:await varInfo(nv),rawValue:node.cornerRadius,bindType:"radius"});}}
    if("opacity"in node&&node.opacity<1&&node.opacity>0){var b=bv.opacity;if(!b){var nv=await findNearestFloatVar(node.opacity,floatVars);checks.opacityVars.issues.push({id:node.id,label:'"'+trunc(node.name)+'" — '+Math.round(node.opacity*100)+'%',path:path,suggestedVar:await varInfo(nv),rawValue:node.opacity,bindType:"opacity"});}}
    ...
    if("strokes"in node&&Array.isArray(node.strokes)&&node.strokes.some(function(s){return s.visible!==false;})&&"strokeWeight"in node&&node.strokeWeight!==figma.mixed&&node.strokeWeight>0){var rawBV=node.boundVariables||{};var bwBound=rawBV.strokeWeight||rawBV.strokeTopWeight||rawBV.strokeBottomWeight||rawBV.strokeLeftWeight||rawBV.strokeRightWeight||bv.strokeWeight||bv.strokeTopWeight||bv.strokeBottomWeight||bv.strokeLeftWeight||bv.strokeRightWeight;if(!bwBound){var nv=await findNearestFloatVar(node.strokeWeight,floatVars);checks.borderVars.issues.push({id:node.id,label:'"'+trunc(node.name)+'" — '+node.strokeWeight+'px',path:path,suggestedVar:await varInfo(nv),rawValue:node.strokeWeight,bindType:"borderWidth"});}}
